...
export default function SpotifyForYou() {
  ...
  useEffect(() => {
    ...
    try { setDismissed(localStorage.getItem("setlst-spotify-banner-dismissed") === "1"); } catch { /* ignore */ }
  }, []);

  function dismiss() {
    try { localStorage.setItem("setlst-spotify-banner-dismissed", "1"); } catch { /* ignore */ }
    setDismissed(true);
  }
    ...
    return (
      <div className="hidden sm:flex relative bg-gradient-to-r from-[#1DB954]/15 to-[#1a1a1a] border border-[#1DB954]/30 rounded-2xl py-4 pl-4 pr-9 items-center gap-3">
        <button
          onClick={dismiss}
          aria-label="Dismiss"
          className="absolute top-1.5 right-1.5 p-1 text-[#a0a0a0] hover:text-[#f0f0f0] transition-colors"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="5" y1="5" x2="19" y2="19" /><line x1="19" y1="5" x2="5" y2="19" /></svg>
        </button>
