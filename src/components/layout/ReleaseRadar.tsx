      ...
      {loading ? (
        ...
        <div className="space-y-3 max-h-[520px] overflow-y-auto">
          {releases.slice(0, 15).map((r) => (
            // The whole row opens the release in Spotify. It used to hang off a
            // 10px "Listen" caption, which is a small target next to 56px of
            // artwork that looked tappable and wasn't.
            <a
              key={r.id}
              href={r.spotifyUrl}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`Play ${r.title} by ${r.artist} on Spotify`}
              className="flex gap-3 press-soft"
            >
              {r.artwork ? (
                <img
                  src={r.artwork}
