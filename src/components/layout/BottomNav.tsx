...
export default function BottomNav() {
  ...
  return (
    <nav className="bottom-nav sm:hidden fixed inset-x-4 bottom-[calc(0.5rem+env(safe-area-inset-bottom))] z-50">
      ...
      <div className="flex items-stretch justify-around h-[2.875rem] rounded-full border border-[#2e2e2e] bg-[#161616] shadow-2xl shadow-black/60 px-1">
        {NAV.map(({ href, label, icon, plus, profile }) => {
          ...
          if (plus) {
            return (
              <Link key={href} href={href} aria-label={label} className="flex items-center justify-center flex-1">
                <span className="flex items-center justify-center w-10 h-10 -mt-1.5 rounded-full bg-[#c4a832] text-[#111111] shadow-lg shadow-black/40 active:scale-95 transition-transform">
                  {icon}
                </span>
              </Link>
            );
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center gap-0.5 flex-1 text-[9px] tracking-wide transition-colors ${
                active ? "text-[#c4a832]" : "text-[#6b6b6b] hover:text-[#a0a0a0]"
              }`}
            >
              {profile && username ? (
                <span className={`rounded-full ${active ? "ring-2 ring-[#c4a832]" : ""}`}>
                  <Avatar username={username} avatar={me?.avatar ?? null} size={19} />
                </span>
              ) : (
                icon
              )}
