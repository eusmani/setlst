...
export default function ReviewVotes({
  reviewId, initialLikes, initialDislikes, initialMyVote, isLoggedIn, shareUrl, shareText,
}: Props) {
  ...
  async function share() {
    const full = `${typeof window !== "undefined" ? window.location.origin : ""}${shareUrl}`;
    if (navigator.share) {
      try { await navigator.share({ title: "SETLST review", text: shareText, url: full }); return; } catch {}
    }
    try {
