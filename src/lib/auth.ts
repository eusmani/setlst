...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      // Keep this lightweight (token only, no DB) — it runs on every auth() call,
      // and a slow/failing query here logs users out server-side. The navbar fetches
      // the live avatar + username separately via /api/me.
      if (token) {
